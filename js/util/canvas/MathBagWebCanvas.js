...
import { getStrImgSrcStr } from '../commonUtils'
...
<body>
  ...
  <script>
    ...
    //         }
    //     });
    //     window.ReactNativeWebView.postMessage(JSON.stringify({action: 8}));
    ...
    function createImg(data){
      ...
      img.onload = function (){
        ...
        ctx.drawImage(img, 150, 120, 400, 200);
      };
      // $(img).attr('src', data);
    }
    ...
    */
    ...
    /** canvas生成图片 */
    function canvasConvertImg(){
      //新Image对象,可以理解为DOM;
      var image = new Image();
      //canvas.toDataURL返回的是一串Base64编码的URL,当然,浏览器自己肯定支持
      //指定格式PNG
      image.src = $can[0].toDataURL("image/png");
      window.ReactNativeWebView.postMessage(JSON.stringify({action: 10, data: $can[0].toDataURL("image/png")}));
    }
...
class WebCanvas extends Component {
  ...
  clear = () => {
    ...
    // console.log('webcanvas clear')
    // if(data.pixels){
    //   this._addImageUrl(getMathPicTopaicUrl(data.private_exercise_stem))
    // }
    // }
  ...
  messageHandler(e) {
    ...
    } else if (obj.action == 8) {
      const { currentTopaicData } = this.props;
      if (currentTopaicData) {
        const data = currentTopaicData.toJS();
        this._addImageUrl(getStrImgSrcStr(data.private_exercise_stem))
      }
    }
  }
  nextTopaic = () => {
