...
class PayModal extends React.Component {
    ...
    // buyEvent = (transactionId, transactionReceipt, pId, subscribeTimesArrIndex) => {
    //     const data = {
    ...
    //             Toast.info("您已成功完成购买!")
    //         })
    //         // log("pay res: ", res.data, res.data.err_code)
    //         // if (res.data.err_code !== 0) {
    //         //     Alert.alert(`购买异常: ${res.data.err_msg}`)
    //         // } else {
    //         //     this.setState({
    //         //         showLoading: false,
    //         //         selectGrade: null,
        ...
        IAP.initConnection().catch(() => {
            ...
            this.purchaseUpdatedListener = IAP.purchaseUpdatedListener(async (purchase) => {
                try {
                    ...
                    const { pId, subscribeTimesArrIndex } = this.state
                    if (pId !== -1) {
                        this.buyInfo = {
                            transactionId,
                            transactionReceipt,
                            pId,
                            subscribeTimesArrIndex,
                        }
                        // this.buyEvent(transactionId, transactionReceipt, pId, subscribeTimesArrIndex)
