...
class PayModal extends React.Component {
    constructor() {
        ...
        this.buyInfo = {}
        this.handleBuyEventThrottled = _.throttle(this.buyEvent, 3 * 1000, { 'trailing': false });
    }
    ...
    iapConnectionInit = () => {
        ...
        IAP.initConnection().catch(() => {
            log("err connected to store...")
        }).then(() => {
            log("connected to store...")
            this.purchaseErrorSubscription = IAP.purchaseErrorListener(
                (error) => {
                    Alert.alert("购买异常，请稍后重试！")
                    this.setState({
                        showLoading: false,
                    })
                },
            );
            this.purchaseUpdatedListener = IAP.purchaseUpdatedListener(async (purchase) => {
                try {
    ...
    requestSubscription = async (productId, quantity) => {
        try {
            ...
            await IAP.requestSubscription({sku: productId, quantity: quantity})
        } catch (err) {
            log("ERROR requestSubscription")
            this.setState({
                showLoading: false,
            })
        }
    ...
    purchase = () => {
        if (Platform.OS==="ios") {
            ...
            const {selectProduct, selectProductId, pId} = this.state
            if (selectProduct === "") {
                Alert.alert("该商品暂不存在～")
            } else {
                if (parseInt(showProductIapPrice) !== 0) {
                    // Alert.alert(`即将购买 ${selectProduct} iapID: [${selectProductId}],  小程序ID:[${pId}]`)
                    this.setState({
                        showLoading: true,
                    })
                    this.requestSubscription(selectProductId, 1)
                } else {
    ...
    render() {
        ...
        const {imgInfoList, subject, selectProduct, showLoading, showProductIapPrice, productInfoList} = this.state
