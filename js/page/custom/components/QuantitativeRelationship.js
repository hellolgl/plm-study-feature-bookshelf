...
class QuantitativeRelationship extends React.Component {
	...
	judgeAnswer = () => {
		...
		this.isTrue = _.isEqual(question, answer);
	}

	updateData = (rawData) => {
		const {answer_contents, options, stem, displayed_type, exercise_type, right_count} = rawData
		const displayed_type_name = displayed_type
		const exercise_type_name = exercise_type
		// option => tto
		// answer => tta
		const tto = options
		const tta = answer_contents
		this.parseHTML(stem)
		const answerDict = {}
		tto.map(t => {
...
export default QuantitativeRelationship
