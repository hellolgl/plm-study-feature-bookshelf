...
class DoExercise extends PureComponent {
    ...
    componentWillUnmount() {
        DeviceEventEmitter.emit("refreshPage", "modular1"); //返回页面刷新
    ...
    getData() {
        ...
        data.days = this.days;
        axios
            .get(api.getEngTopicDairy, { params: data })
            .then((res) => {
                const data = res.data.data;
                data.forEach((item, index) => {
                    data[index] = changeCurrentTopic(item);
                });
                this.setState(
                    () => ({
                        topicList: data,
                        currentTopic: data.length > 0 ? data[0] : {},
                        lt_diagnosis_id: data.length > 0 ? data[0].lt_diagnosis_id : "",
                    }),
                    () => {
    ...
    submit = () => {
        ...
        const { currentTopic, topicIndex, topicList } = this.state;
        ...
        if (this.do_num === 1) {
            ...
            if (topicIndex + 1 === topicList.length) this.saveThisExercise();
            switch (ranking) {
                case "0":
                    this.ranking_0_num++;
                    break;
