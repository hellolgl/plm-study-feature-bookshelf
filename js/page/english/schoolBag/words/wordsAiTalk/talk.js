...
function MyBubble(props) {
    ...
    return (
        <TtsPlayAudio disablePlay={isUser || renderLottie} text={text} language='en-IE'>
            <View
                style={[
                    styles.bubbleWrap,
                    isUser
        ...
        const fetchData = async () => {
            if (!messages.length) {
                ...
                if (chatLoading === false) {
                    const history_res = await axios.post(api.getCheckWordsAIBotHistory, data);
                    ...
                    // });
                    try {
                        const res = await axios.post(api.getCheckWordsAiHelp, data)
