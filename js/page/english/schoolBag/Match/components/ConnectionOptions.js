...
class ConnectionOptions extends PureComponent {
    ...
    static getDerivedStateFromProps(props, state) {
        ...
        if (props.currentTopic.l_p_id !== state.currentTopic.l_p_id || Object.keys(props?.currentTopic).length > 0 && isRender) {
            let tempState = { ...state };
            ...
            tempState.isLeft = props.currentTopic.combination_type === "1" ? false : true; //1上下   2左右
    ...
    checkValue = (index, leftOrRight, value) => {
        // console.log('点击::::::::::', index, leftOrRight, value)
    ...
    getWrongList = () => {
        ...
        if (!token && this.props.resetToLogin) {
            this.props.resetToLogin(this.props);
            return
        }
        ...
        const { doWrong } = this.props
        if (doWrong) {
            if (wrongCanvasList.length > 0 || canvasList.length < rightList.length) {
                // 错
                this.props.showHelp()
                ...
                this.props.initTopic && this.props.initTopic()
            }
        } else {
            ...
            if (this.clickNumber === 1) {
                ...
                // 有错误连线或者连的线不够算错
                if (canvasList.length < rightList.length) {
                    this.props.saveTopic && this.props.saveTopic([{}], this.topicIndex + 1 === exerciseLen);
                } else {
                    this.props.saveTopic && this.props.saveTopic(wrongCanvasList, this.topicIndex + 1 === exerciseLen);
                }
            }
    ...
    tryAgian = () => {
        ...
        if (this.noNumber === 2) {
            if (this.topicIndex + 1 === exerciseLen) {
                // 已经是最后一题了
                this.props.showStatistic && this.props.showStatistic();
                return;
            }
            this.topicIndex++;
            this.init();
        ...
        console.log("题目题号", this.topicIndex, exerciseLen);
        this.setState({
            isRender: true,
        });
    }
    ...
    renderLeft = () => {
        const { currentTopic, leftList, corlorIndex, isLeft, nowCheckObj } = this.state;
        let html = "";
                ...
                return (
                    <TouchableOpacity
                        style={[styles.boxStyle, {
                            backgroundColor: nowCheckObj.left === item.value ? '#447BE5' : '#E7E7F2',
                        }]}
                        onPress={() => this.checkValue(index, "left", item.value)}
                        key={index}
                        onLayout={(e) => this.onLayoutItem("leftItem", e, index)}
                    >
                        <View style={[styles.boxStyleIn, { backgroundColor: nowCheckObj.left === item.value ? '#4C94FF' : '#F5F5FA' }]}>
                            <Text style={[{ color: nowCheckObj.left === item.value ? "#fff" : '#475266', fontSize: pxToDp(40), lineHeight: pxToDp(50) }, appFont.fontFamily_syst, fonts.fontFamily_jcyt_700]}>
                                {item.value}
                            </Text>
                        </View>
                    </TouchableOpacity>
        ...
        if (currentTopic.combination_one === "audio") {
            html = leftList.map((item, index) => {
                ...
                return (
                    <View key={index} onLayout={(e) => this.onLayoutItem("leftItem", e, index)}>
                        <PlayAudioBtn audioUri={baseUrl + item} onPress={() => {
                            this.checkValue(index, "left", item.value)
                        }}>
                            <View
                                style={[styles.boxStyle, {
                                    backgroundColor: nowCheckObj.left === item ? '#447BE5' : '#E7E7F2'
                                }]}
                                onLayout={(e) => this.onLayoutItem("leftItem", e, index)}
                            >
                                <View style={[styles.boxStyleIn, { backgroundColor: nowCheckObj.right === item ? '#4C94FF' : '#F5F5FA' }]}>
                                    <Image
                                        style={[size_tool(nowCheckObj.left === item.value ? 40 : 80)]}
                                        source={nowCheckObj.left === item ? require("../../../../../images/chineseHomepage/pingyin/new/audioPlaying.png") : require("../../../../../images/chineseHomepage/pingyin/new/audioPlay.png")}
                                        resizeMode={"contain"}
                                    ></Image>
                                </View>
                            </View>
    ...
    renderRight = () => {
        const { currentTopic, rightList, corlorIndex, isLeft, nowCheckObj } = this.state;
        let html = "";
        ...
        if (currentTopic.combination_two === "text") {
            html = rightList.map((item, index) => {
                ...
                return (
                    <TouchableOpacity
                        style={[styles.boxStyle, {
                            backgroundColor: nowCheckObj.right === item ? '#447BE5' : '#E7E7F2'
                        }]}
                        onPress={() => this.checkValue(index, "right", item)}
                        key={index}
                        onLayout={(e) => this.onLayoutItem("rightItem", e, index)}
                    >
                        <View style={[styles.boxStyleIn, { backgroundColor: nowCheckObj.right === item ? '#4C94FF' : '#F5F5FA', paddingLeft: pxToDp(15), paddingRight: pxToDp(15) }]}>
                            <Text style={[{ color: nowCheckObj.right === item ? "#fff" : '#475266', fontSize: pxToDp(40), lineHeight: pxToDp(50) }, appFont.fontFamily_syst, fonts.fontFamily_jcyt_700]}>
                                {item}
                            </Text>
                        </View>
                    </TouchableOpacity>
        ...
        if (currentTopic.combination_two === "audio") {
            html = rightList.map((item, index) => {
                return (
                    <View key={index} onLayout={(e) => this.onLayoutItem("rightItem", e, index)}>
                        <PlayAudioBtn audioUri={baseUrl + item} onPress={() => {
                            this.checkValue(index, "right", item)
                        }}>
                            <View
                                style={[styles.boxStyle, {
                                    backgroundColor: nowCheckObj.right === item ? '#447BE5' : '#E7E7F2'
                            ...
                            >
                                <View style={[styles.boxStyleIn, { backgroundColor: nowCheckObj.right === item ? '#4C94FF' : '#F5F5FA' }]}>
                                    <Image
                                        ...
                                        source={nowCheckObj.right === item ? require("../../../../../images/chineseHomepage/pingyin/new/audioPlaying.png") : require("../../../../../images/chineseHomepage/pingyin/new/audioPlay.png")}
                                        resizeMode={"contain"}
                                    ></Image>
                                </View>
                            </View>
    ...
    render() {
        ...
        return (
            <View style={[styles.container]}>
                    ...
                    style={[
                        {
                            ...
                            width: isLeft ? "80%" : "100%",
                            // width:'100%',
                            height: Platform.OS === "ios" ? Dimensions.get("window").height * 0.6 : Dimensions.get("window").height * 0.55,
                            marginTop: pxToDp(20),
                ...
                <TouchableOpacity onPress={this.getWrongList} style={[styles.nextBtn, appStyle.flexCenter, appStyle.flexLine]}>
                    {/* <Text style={[styles.nextBtnText, appFont.fontFamily_syst]}>{this.props.isdid ? '完成' : '下一题'}</Text> */}

                    <ImageBackground style={[size_tool(200), appStyle.flexCenter]}
                ...
                >
                    <Surface width={Dimensions.get("window").width * 0.9} height={Dimensions.get("window").height * 0.7} style={{}}>
                        <Group>
                            {lineViewArr}
                        </Group>
                    </Surface>
...
export default connect(mapStateToProps, mapDispathToProps)(ConnectionOptions);
