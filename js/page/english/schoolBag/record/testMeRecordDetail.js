...
import { Toast, Radio, Modal } from 'antd-mobile-rn'
...
class SynchronizeDiagnosisEn extends Component {
    ...
    constructor(props) {
        ...
        this.handlenOnCloseHelpThrottled = _.throttle(this.onCloseHelpNow, 3 * 1000);
    ...
    componentDidMount() {
        ...
        //console.log('获取题目')
        axios.get(api.getEnglishTestMeRecodListDetail, { params: data }).then(
            res => {
    ...
    componentWillUnmount() {
        // console.log('this.saveEnBagExerciseTimer', this.props.navigation.state.params.data)
        ...
        this.closeDialog()
        this.closeAudio()
        if (this.props && this.props.navigation && this.props.navigation.state.params.data.isUpload) {
    ...
    playHeplAudio = () => {
        ...
        this.audioHelp = new Sound(url + fromServeCharacterList[topaicIndex].explanation_audio, null, (error) => {
            if (error) {
    ...
    nextTopaic = () => {
        ...
        const { fromServeCharacterList, topaicIndex, imgMCQAnswer, optionList } = this.state;
        ...
        else if (optionList[index].trim() === fromServeCharacterList[topaicIndex].answer_content.trim()) {
            // 正确，当前题目为推送的要素题时不论对错都保存
    ...
    helpMe = (isHelp) => {
        ...
        data.knowledge_type = this.props.navigation.state.params.data.node === 3 ? 1 : 2  //1.字母单元 2.非字母单元
        if (fromServeCharacterList[topaicIndex].knowledgepoint_type === '4' && fromServeCharacterList[topaicIndex].exercise_type_public === 'Dialogue') {
            this.setState(() => ({
    ...
    onCloseHelp = () => {
        ...
        console.log('this.state.optionList', this.state.optionList, this.state.checkedIndex)
        this.isSave ? this.saveEnBagExercise(1,
    ...
    onCloseHelpNow = () => {
        ...
        this.isSave ? this.saveEnBagExercise(1,
            fromServeCharacterList[topaicIndex].choice_content_type === 'image' ?
                imgMCQAnswer
                :
                optionList[this.state.checkedIndex].trim(), false) : ''
        this.isSave = false
        this.closeHelpAudio()
        this.setState({
            articleStatus: false
    ...
    getDataFromCanvas = (canvasData) => {
        ...
        if (this.props.navigation.state.params.data.knowledge_type == 1) {
            ...
            if (this.judgeAnswerContentType1(fromServeCharacterList[topaicIndex].answer_content)) {
                if (answerArr[0].toLocaleLowerCase() === fromServeCharacterList[topaicIndex].answer_content.toLocaleLowerCase() && answerArr[1] === 1) {
                    //字母与正确答案一致，并且字母笔顺正确
                ...
                if (answerArr[0] === fromServeCharacterList[topaicIndex].answer_content && answerArr[1] === 1) {
                    //字母与正确答案一致，并且字母笔顺正确
                    this.saveExerciseDetail(0, canvasData, true)
                    fromServeCharacterList[topaicIndex].colorFlag = 1
                } else {
    ...
    renderMCQType = (data) => {
        console.log("data.exercise_element: ", data.exercise_element)
    ...
    renderFTBType = (data) => {
        console.log("FTB,  data.exercise_element: ", data.exercise_element)
        switch (data.exercise_element) {
            case EngTypes.L:
    ...
    renderImgMCQ = (data) => {
        ...
        const { private_exercise_stem, fromServeCharacterList, topaicIndex, isRenderOptions, optionList } = this.state;
        ...
        data.private_exercise_stem = data.private_exercise_stem.replaceAll('xx-large', '48px')
        let optionListnow = [...optionList]
        if (isRenderOptions) {
            let optionListnow = data.choice_content ? this.renderOptions(data.choice_content) : []
            this.setState({
        ...
        return (
            ...
            <View style={{ flexDirection: 'row', backgroundColor: '#FFFFFF', flex: 1, borderRadius: pxToDp(32) }}>
                <View style={{ padding: pxToDp(40), borderRadius: pxToDp(32) }}>
                    ...
                    <View style={{ display: 'flex', flexDirection: 'row' }}>
                        {data.choice_content ? optionListnow.map((item, index) => {
                            if (data.choice_content_type === 'image') {
                                return (
                                    <View style={{ flexDirection: 'row' }}>
                                        <Radio
                                        ...
                                        <TouchableOpacity onPress={() => this.checkAnswerImg(item, fromServeCharacterList[topaicIndex].private_exercise_stem, index)} key={index}>
                                            <View style={{ display: 'flex', flexDirection: 'row', flexWrap: 'wrap', marginRight: 20, marginTop: 10, alignItems: 'center' }}>
                                                <Image style={{ width: pxToDp(200), height: pxToDp(200), marginRight: pxToDp(20), resizeMode: 'contain' }} source={{ uri: 'https://pailaimi-static.oss-cn-chengdu.aliyuncs.com/' + item }}></Image>
                                            </View>
                                        </TouchableOpacity>
            ...
            {data.private_stem_picture ?
                <View style={{ backgroundColor: '#fff', height: pxToDp(360), borderRadius: pxToDp(32), marginTop: pxToDp(30) }}>
                    <Image style={{ width: pxToDp(300), height: pxToDp(300), resizeMode: 'contain', marginLeft: pxToDp(32) }} source={{ uri: baseUrl }}></Image>
                    {this.renderButton()}
                </View>
                :
                null
    ...
    renderCommonMCQ = (data) => {
        let baseUrl = 'https://pailaimi-static.oss-cn-chengdu.aliyuncs.com/' + data.private_stem_picture
        const { fromServeCharacterList, topaicIndex, isRenderOptions, optionList } = this.state;
        let optionListnow = [...optionList]
        if (isRenderOptions) {
            let optionListnow = data.choice_content ? this.renderOptions(data.choice_content) : []
            this.setState({
        ...
        return (
            <View style={{height: "100%"}}>
                <View style={[{ flexDirection: 'row', paddingLeft: pxToDp(24), borderRadius: 15, backgroundColor: "white", flex: 1, }]}>
                        ...
                        <ScrollView >
                            <View style={{ paddingRight: pxToDp(40) }}>
                                ...
                                {fromServeCharacterList[topaicIndex] && fromServeCharacterList[topaicIndex].public_stem_picture ?
                                    <View style={{ borderRadius: 15, width: 200, height: 150, backgroundColor: '#FFFFFF', marginRight: pxToDp(12) }}>
                                        <Image style={{ width: 200, height: 150, resizeMode: 'contain', borderRadius: 15, }} source={{ uri: 'https://pailaimi-static.oss-cn-chengdu.aliyuncs.com/' + fromServeCharacterList[topaicIndex].public_stem_picture }}></Image>
                                    </View> : null}
                            </View>
                            {fromServeCharacterList[topaicIndex] && fromServeCharacterList[topaicIndex]?.public_exercise_stem ? <View style={{ height: pxToDp(48), width: pxToDp(48) }}></View> : null}
                            <View style={{ flexDirection: 'row' }}>
                                {data.private_stem_audio ? (
                                    <TouchableOpacity
                ...
                {data.private_stem_picture ?
                    <View style={{ flexDirection: 'row', marginTop: pxToDp(36) }}>
                        <View style={{ alignItems: "center", justifyContent: "center", borderRadius: 15, width: Dimensions.get("window").width * 0.25, height: Dimensions.get("window").height * 0.33, backgroundColor: 'white', }}>
                            <Image style={{ width: Dimensions.get("window").width * 0.2, height: Dimensions.get("window").height * 0.30, resizeMode: 'contain', borderRadius: 15, }} source={{ uri: baseUrl }}></Image>
                        </View>
                        <View style={{ backgroundColor: '#fff', width: Dimensions.get("window").width * 0.67, borderRadius: 15, paddingLeft: pxToDp(24), paddingTop: pxToDp(24), marginLeft: "auto"}}>
                            {data.choice_content ? optionListnow.map((item, index) => {
                                if (data.choice_content_type === 'image') {
                                    return (
                                        <TouchableOpacity onPress={() => this.checkAnswer(index)} key={index}>
                                            <View style={{ display: 'flex', flexDirection: 'row', flexWrap: 'wrap', marginBottom: 20, alignItems: 'center' }}>
                                                <View style={this.state.checkedIndex === index ? styles.checkedOption : styles.checkOptions}>
                                                    <Text style={{ color: this.state.checkedIndex === index ? '#FFFFFF' : '#666666', fontSize: pxToDp(32), }}>{zimu[index]}</Text>
                                                </View>
                                                <Image style={{ width: 300, height: 300, marginLeft: 20, marginBottom: pxToDp(12), resizeMode: 'contain' }} source={{ uri: 'https://pailaimi-static.oss-cn-chengdu.aliyuncs.com/' + item }}></Image>
                                            </View>
                                        </TouchableOpacity>)
                                } else {
                                    return (
                                        <TouchableOpacity onPress={() => this.checkAnswer(index)} key={index}>
                                            <View style={{ display: 'flex', flexDirection: 'row', flexWrap: 'wrap', marginBottom: 20, alignItems: 'center' }}>
                                                ...
                                                <Text style={{ marginRight: 10, fontSize: 24 }}>{item}</Text>
                                            </View>
                                        </TouchableOpacity>
                                    )
                                }
                    ...
                    :
                    <View style={{ flexDirection: 'row', marginTop: pxToDp(36), height: pxToDp(380) }}>
                        <View style={{ backgroundColor: '#fff', width: '100%', borderRadius: 15, paddingLeft: pxToDp(24), paddingTop: pxToDp(24) }}>
                            {data.choice_content ? optionListnow.map((item, index) => {
                                if (data.choice_content_type === 'image') {
                                    return (
                                        <TouchableOpacity onPress={() => this.checkAnswer(index)} key={index}>
    ...
    renderSType = (data) => {
        ...
        const { fromServeCharacterList, topaicIndex } = this.state;
        return (
            <View style={{ backgroundColor: '#FFFFFF', borderRadius: 15, padding: 24, height: "100%" }} >
                <View style={{ flexDirection: 'row', }}>
                    <RenderHtml
                        source={{html:fromServeCharacterList[topaicIndex] && fromServeCharacterList[topaicIndex]?.public_exercise_stem ? fromServeCharacterList[topaicIndex].public_exercise_stem : ' '}}
                        tagsStyles={{ p: { fontSize: 24 }, div: { fontSize: 24 }, span: { fontSize: 24 } }}
                ...
                {data.private_stem_picture ?
                    <Image style={{ marginTop: pxToDp(48), width: pxToDp(450), height: pxToDp(375), resizeMode: 'contain' }} source={{ uri: baseUrl }}></Image> : <Text></Text>
                }
    ...
    renderWriteTopaic = () => {
        ...
        const data = { ...map.get(topaicIndex) };
        console.log("debug 11111: ", data.exercise_type_public)
        // console.log('data', data)
        if (!data.exercise_id) {
            return (
    ...
    renderButton = () => {
        return (<TouchableOpacity style={styles.topaicBtn} ref='topaicBox' onLayout={(event) => this.topaicContainerOnLayout(event)} onPress={this.handelNextTopic}>
            <View style={{ width: 90, height: 40, borderRadius: 50, backgroundColor: '#38B3FF', alignItems: 'center', justifyContent: "center" }}><Text style={{
    ...
    render() {
        const { topaicNum, fromServeCharacterList, topaicIndex, knowledgeBody, isStartAudio } = this.state
        return (
            <View style={styles.mainWrap}>
                <View>
                    <Header text={this.props.navigation.state.params.data.unit_name} txtStyle={{ fontWeight: 'bold' }} goBack={() => { this.goBack() }}></Header>
                </View>
                ...
                <EngLishCanvasDialog
                    ...
                    _dialogRightBtnAction={this.clear} _dialogLeftBtnAction={this.handelNextTopic}
                >
                ...
                <Modal
                    ...
                    footer={[{ text: "Close", onPress: () => this.setState({ goodVisible: false }) }]}
                ...
                <Modal
                    ...
                    transparent
                    // maskClosable={false}
                    visible={this.state.articleStatus}
                    footer={[{ text: "关闭", onPress: this.handlenOnCloseHelpThrottled }]}
                    style={{ width: pxToDp(1200) }}
                >
                    <View style={{ paddingVertical: 20, maxHeight: pxToDp(400) }}>
                        <ScrollView>
...
const styles = StyleSheet.create(
    {
        mainWrap: {
            ...
            height: '100%',
            width: '100%',
            padding: Platform.OS === "ios"? pxToDp(60): pxToDp(40),
...
export default connect(mapStateToProps, mapDispathToProps)(SynchronizeDiagnosisEn)
