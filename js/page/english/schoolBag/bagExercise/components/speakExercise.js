...
class DoWrongExercise extends PureComponent {
    ...
    renderWriteTopaic = () => {
        const { exerciseInfo, scoreTxt, isRecording } = this.state;
        ...
        if (!iseSentence) {
            iseSentence = doc.documentElement.querySelector("b")?.innerText();
        }
        if (!iseSentence) {
                ...
                >
                    ...
                    >
                        {isRecording ? null : (
                            <Text
                                style={[
                                    { fontSize: pxToDp(80), color: "#00D3A1", opacity: 0.5 },
