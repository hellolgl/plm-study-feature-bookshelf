...
class DoExercise extends PureComponent {
  ...
  componentWillUnmount(){
    DeviceEventEmitter.emit("refreshPage",this.state.currentTopic.page_index); //返回页面刷新
  }
  ...
  saveData = () =>{
    const {currentTopic} = this.state
      // 答对了才保存
    // console.log('单题保存数据',currentTopic.m_de_e_id)
    axios.post(api.saveMathDeskWrongTopic, {m_de_e_id:currentTopic.m_de_e_id}).then((res) => {
  ...
  render() {
    ...
    return (
      <ImageBackground style={[styles.container]} source={require('../../../images/MathSyncDiagnosis/bg_1.png')}>
        ...
        <Text style={[styles.header]} onLayout={(e) => this.onLayoutHeader(e)}>{onlySeeExplain?'错题查看':'错题再练'}</Text>
        ...
        {isWrong && displayed_type_name === topaicTypes.Application_Questions?<View style={[styles.explanation_wrap,{height:explanation_height,paddingLeft:pxToDp(0),paddingRight:pxToDp(0)}]}>
          <ApplicationFillBlank  my_style={style} doWrong={true} data={currentTopic} tryAgain={this.tryAgain}></ApplicationFillBlank>
        </View>:null}
