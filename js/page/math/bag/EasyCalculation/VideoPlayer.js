...
export default class VideoPlayer extends Component {
  ...
  renderRateControl = (rate) => {
    ...
    return (
      <TouchableOpacity onPress={() => { this.setState({ rate }) }}>
        <Text style={[{ "fontSize": 19, "paddingLeft": 30, }, { fontWeight: isSelected ? 'bold' : 'normal' }, { color: isSelected ? '#0179FF' : 'white' }]}>
          {rate}x
        </Text>
      </TouchableOpacity>
    )
  ...
  renderTimer() {
    return this.renderControl(
      <Text style={styles.controls.timerText}>{`${this.formatTime(this.state.currentTime)} / ${this.formatTime(this.state.duration)}`}</Text>,
    ...
    return (
      <TouchableWithoutFeedback
        ...
        <View style={[styles.player.container, this.styles.containerStyle]}>
          ...
          // source={fileUrl}
          // source={require("../../../../images/chineseHomepage/pingyin/new/1.mp4")}
          />
...
const styles = {
  ...
  seekbar: StyleSheet.create({
    ...
    icon: {
      top: pxToDp(0),
      height:Platform.OS === 'android'? pxToDp(50):pxToDp(70),
      width: Platform.OS === 'android'? pxToDp(50):pxToDp(70),
      resizeMode: "contain",
    },
    track: {
      // 未播放的进度条颜色
