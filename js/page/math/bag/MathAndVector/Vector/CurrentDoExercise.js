...
class CurrentDoExercise extends Component {
    constructor(props) {
        super(props)
        this.name = this.props.navigation.state.params.data.name
        this.g_b_id = this.props.navigation.state.params.data.g_b_id
        this.yesNum = 0
        this.wrongNum = 0
        this.correctIdArr = []
        this.state = {
    ...
    componentDidMount(){
        ...
        axios.get(api.getMathVectorCurrentTopicStatus, { params: {g_b_id:this.g_b_id}}).then((res) => {
    ...
    getTopic = (data)=>{
        ...
        if(data.type === 'dead'){
            // 死题
            axios.get(api.getMathVectorCurrentTopic, { params: obj}).then((res) => {
                let data = res.data.data
            ...
            // 活题
            axios.get(api.getMathVectorCurrentTopicOperat, { params: obj}).then((res) => {
                let data = res.data.data
                console.log('活题',data)
                this.setState((prevState, props)=>({
                    currentTopaicData:data,
...
export default CurrentDoExercise