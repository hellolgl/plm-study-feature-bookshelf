...
class DoExercise extends PureComponent {
    ...
    getData = () => {
        const { lesson_code, status } = this.props.navigation.state.params.data;
        ...
        axios
            .get(api.getMathSyncDiagnosisTopicNum, { params: data })
            .then((res) => {
                ...
                if (data.length > 0) {
                    ...
                    m_e_s_id = data[_topicIndex].m_e_s_id;
                }
                this.setState(
                    {
                        topic_num_list: data,
    ...
    getTopic = () => {
        ...
        axios.get(api.getMathSyncDiagnosisTopic, { params: data }).then((res) => {
    ...
    saveData = (data) => {
        axios.post(api.saveMathSyncDiagnosisTopic, data).then((res) => {
    ...
    next = () => {
        const { topic_num_list, currentTopic } = this.state;
    ...
    toKnowlegdeDetails = () => {
        ...
        this.props.setLanguageData(language_data)  //这个入口强行设置成中文
        const origin = this.props.navigation.state.params.data.origin
        const { currentTopic } = this.state
        const { knowledge_name, knowledge_code } = currentTopic
        const data = {
            knowledge_code,
            knowledge_name,
            origin,
        ...
        this.props.setToKnowledge(true)
    ...
    render() {
        ...
        return (
            ...
            >
                {/* 应用题流程答错填空的时候ai按钮不展示 */}
                {isWrong && displayed_type_name === topaicTypes.Application_Questions ? null : <TouchableOpacity style={{ position: "absolute", right: pxToDp(256), top: pxToDp(160), zIndex: 1 }} onPress={this.toKnowlegdeDetails}>
                    <Image style={[{ width: pxToDp(139), height: pxToDp(116) }]} source={require('../../../../images/MathSyncDiagnosis/btn_bg_6.png')}></Image>
                </TouchableOpacity>}
