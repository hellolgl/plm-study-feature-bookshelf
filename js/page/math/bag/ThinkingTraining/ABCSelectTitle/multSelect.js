...
class MultSelectTitle extends PureComponent {
    constructor(props) {
        super(props)
        this.state = {
            stem: "",
    ...
    selectAnswer = (i) => {
        let {selectAnswer} = this.state
        let n
        if (selectAnswer.includes(i)) {
            n = selectAnswer.filter(d => d !== i)
        } else {
            n = Array.from(new Set([...selectAnswer, i]))
        }
        n.sort()
        this.judgeAnswer(n.join(","))
        this.setState({
            selectAnswer: n
        })
    }
        ...
        return (
            <View style={[{"maxWidth": maxWidth}, styles.optionPosition]}>
                {option.map((d, index) => {
                    if (d.startsWith("[[")) {
                        let fontColor = "black"
                        if (selectAnswer.includes(this.getSelectOption(index))) {
                            fontColor = "#669FFD"
                        }
                        const fontStyle = {
                            color: fontColor,
                        ...
                        return (
                            <TouchableOpacity
                                ...
                                <Text>
                                    <Text style={[styles.selectOption]}>{this.getSelectOption(index)}. </Text>
                                    {selectAnswer.includes(this.getSelectOption(index))?
...
export default MultSelectTitle
