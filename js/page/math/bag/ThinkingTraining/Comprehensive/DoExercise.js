...
import HelpModal from '../../ExpandApplication/components/HelpModal'
...
import Explanation from '../../ExpandApplication/components/Explanation'
...
class EasyCalculationDoExercise extends PureComponent {
  ...
  getData = () => {
    ...
    let obj = {
      ...
      t_t_id:this.t_t_id,
      answer_type:answer_type_map[listIndex]
    }
    axios.get(api.getMathThinkingComprehensiveTopic, { params: obj }).then(
      res => {
        let data = res.data.data.thinking_data
        data.answer_type = answer_type_map[listIndex]
        data = changeTopicData(data)
        if(!this.t_h_id) this.t_h_id = res.data.data.t_h_id
        this.setState({
          animating:false,
    ...
    if(isWrong){
      ...
      return <>
          <View style={[appStyle.flexTopLine,appStyle.flexJusBetween]}>
            ...
            {isYinDao?<View style={{position:'absolute',right:0}}>
  ...
  getSaveData = (currentTopaicData)=>{
    ...
    let data = {
      t_h_id:this.t_h_id,
      t_s_id:currentTopaicData.t_s_id,
      textbook:textCode,
      grade_code: userInfoJs.checkGrade,
      term_code: userInfoJs.checkTeam,
      answer_type:answer_type_map[listIndex],
      topic_type:currentTopaicData.topic_type
    }
    return data
  }
  submit = async()=>{
    ...
    // data.exercise_result = currentTopaicData.isFraction?JSON.stringify(diagnosisByFrontVal):exercise_result_fs   //暂时不存作答的答案
  ...
  onFail = (data,info)=>{
    ...
    Toast.info(info, 2,()=>{
      if(this.answerNum === 1){
        this.wrongIdArr.push(currentTopaicData.t_s_id)
  ...
  onSendResult = (data,result)=>{
    ...
    axios.post(api.saveMathThinkingComprehensiveTopic, data).then(res=>{
      if(result){
        // 作答成功
        this.toNextTopic()
      }
  ...
  render() {
    ...
    return (
    <ImageBackground style={[styles.container]} source={require('../../../../../images/thinkingTraining/do_exercise_bg.png')}>
        <Header text={'综合练习'} goBack={this.goBack} seeHelp={this.helpMe}></Header>
              ...
              <View style={[styles.left,isWrong || isOpen?{flex:1,marginRight:0}:null]}>
                ...
                <View style={[styles.leftTwo,isWrong || isOpen?{height:fitHeight(0.807,0.853)}:null]}>
                    ...
                    <View style={[appStyle.flexAliEnd]}>
                      ...
                      </TouchableOpacity>:isOpen?null:<TouchableOpacity style={[styles.baseBtn,appStyle.flexCenter]} onPress={this.submitThrottle}>
                        <Text style={[styles.btnText]}>提交</Text>
                      </TouchableOpacity>}
                    </View>
                </View>
            </View>
            {isWrong || isOpen?null: <View style={[styles.right]}>
