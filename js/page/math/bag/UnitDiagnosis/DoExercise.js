...
import { padding_tool, pxToDp, pxToDpHeight, size_tool } from "../../../../util/tools";
...
class DoExercise extends PureComponent {
    ...
    getData = () => {
        ...
        axios
            .get(api.getUnitDiagnosisTopic, { params: data })
            .then((res) => {
                let data = res.data.data.data;
                data.forEach((i) => {
                    i.m_e_s_id = i.exercise_id
                })
                this.total = data.length;
                this.u_h_id = res.data.data.u_h_id;
                let m_e_s_id = "";
                let _topicIndex = topicIndex;
                if (data.length > 0) {
                    if (status === 1) {
                        // 继续作答
                        let hasAnswered = data.some(i => i.correct === 1 || i.correct === 0);
                        this.setState({ showEmpty: !hasAnswered });
    ...
    saveData = (data) => {
        axios.post(api.getUnitDiagnosisTopicSave, data).then((res) => {
    ...
    render() {
        ...
        return (
            ...
            >
                ...
                {
                    this.state.showEmpty && <View style={[{
                        alignItems: 'center',
                        marginTop: pxToDp(220)
                    }]}>
                        <ImageBackground
                            ...
                            resizeMode={"contain"}
                        >
                            <Text style={[{ fontSize: pxToDp(56), color: '#b75526', fontFamily: Platform.OS === 'ios' ? 'Muyao-Softbrush' : 'Muyao-Softbrush-2', }]}>暂无记录</Text>
                        </ImageBackground>
                    </View>
                }
                {
                    !this.state.showEmpty && <View
                        style={[
                ...
                {
                    !this.state.showEmpty && <View style={[styles.content]}>
