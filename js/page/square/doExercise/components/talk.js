...
function MyBubble(props) {
    const { currentMessage, myBubbleTxtViewStyle, myBubbleUserViewStyle, gptAvatar } = props;
...
function Message(props) {
    ...
    const renderMyAvatar = (role) => {
        const roleAvatarMap = {
            user: avatar,
            gpt: gptAvatar ? gptAvatar : require("../../../../images/square/gpt_avatar.png"),
        };
        return (
            <Image
                resizeMode="stretch"
    ...
    const renderMyBubble = () => {
        ...
        if (!defaultBubble) {
            return <MyBubble {...bubbleProps} myBubbleTxtViewStyle={myBubbleTxtViewStyle} myBubbleUserViewStyle={myBubbleUserViewStyle}></MyBubble>;
        }
...
function Talk({ inputToolbarStyle, myBubbleTxtViewStyle, myBubbleUserViewStyle, gptAvatar }) {
    const { homeSelectItem, messages, canSend, chatLoading, sessionId } = useSelector((state) => state.toJS().square);
    ...
    useEffect(() => {
        ...
        const fetchData = async () => {
            if (!messages.length) {
                ...
                const data = { title: homeSelectItem.title };
                if (chatLoading === false) {
                    ...
                    try {
                        const res = await axios.post(api.getSquareScienceTalk, data);
        ...
        return () => {
            ...
            if (homeSelectItem.module === "common_story_v3") {
                dispatch({
    ...
    const renderMessage = (props) => {
        return <Message {...props} myBubbleTxtViewStyle={myBubbleTxtViewStyle} myBubbleUserViewStyle={myBubbleUserViewStyle} gptAvatar={gptAvatar} defaultBubble={false}></Message>;
    };
