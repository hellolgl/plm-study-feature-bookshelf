...
import SelectLanguageModal from "../../math/bag/KnowledgeGraph/components/SelectLanguageModal";
import BadgeModal from "../../math/bag/KnowledgeGraph/components/BadgeModal";
import DagGraph from "../../math/bag/KnowledgeGraph/components/DagGraph";
...
class Test extends PureComponent {
  ...
  getUnit = () => {
    ...
    let porpsdata = this.props.navigation.state.params.data;
    let params_00 = {
    ...
    Promise.all([
      axios.get(api.getMathKGUnit, { params: params_00 }),
      axios.get(api.getMathKGUnit, { params: params_01 }),
    ]).then((res) => {
      let unit_00 = res[0].data.data;
      let unit_01 = res[1].data.data;
      let arr = [...unit_00, ...unit_01];
      ...
      data.forEach((i, x) => {
        ...
        if (x > 7) {
          i._unit_name = `第${unitList[x]}单元`;
          i._language_unit_name = `Unit ${x + 1}`;
        }
        if (show_type === "1") {
          i.unit_name_z = i._unit_name;
          i.unit_name_c = i._language_unit_name;
  ...
  next = () => {
    ...
    NavigationUtil.toMyChildrenMathKnowExplain({
      ...this.props,
      data: { ...data },
    });
  };
  ...
  gotoDetailPage = (swiperIndex, index) => {
    ...
    NavigationUtil.toMyChildrenMathKnowExplain({ ...this.props, data });
  };
  ...
  render() {
    ...
    const { back_z, back_c, zstupu_z, zstupu_c, free_z, free_c } = page_data;
...
const styles = StyleSheet.create({
  ...
  header: {
    height: pxToDp(120),
    backgroundColor: "#3550B1",
  },
  header_inner: {
    height: pxToDp(110),
    backgroundColor: "#476ED4",
