...
class DoWrongExercise extends PureComponent {
  ...
  playAudio = (path) => {
    ...
    //console.log("播放语音地址", url.baseURL + path);
    // let _path =
    //   url.baseURL + "chinese/03/00/exercise/audio/f0360c89e1bf4feca57b0ee67891df7b.mp3";
    if (audio) {
      audio.stop(() => {
        audio.play();
      });
  ...
  componentDidMount() {
    ...
    axios.get(`${api.getChineseExerciseDetail}/${this.props.navigation.state.params.data.exercise_id}`).then((res) => {
  ...
  renderWriteTopaic = () => {
    ...
    const { exerciseInfo, optionList, playStatus, isStartAudio1, pausedPrivate, paused } = this.state;
                ...
                return (
                  ...
                  >
                    ...
                    >
                      ...
                      <Text style={[{ fontSize: pxToDp(35), flex: 1 }, appFont.fontFamily_syst, fontFamilyRestoreMargin()]}>
                        {item}
                      </Text>
                    </View>
                  </TouchableOpacity>
    ...
    if (isEnd) {
      return (
        ...
        >
          ...
          >
            <Text style={[{ color: "#ffffff", fontSize: pxToDp(32) }, appFont.fontFamily_syst]}>完成</Text>
          </View>
        </TouchableOpacity>
      );
    } else {
  ...
  render() {
    const { exerciseInfo, explanation_audio, isStartAudio, pausedHelp } = this.state;
            ...
            <View style={styles.topaicText}>
              <View style={{ flex: 1 }}>
                <ScrollView>
                  ...
                  {
                    exerciseInfo.public_stem_audio ?
                      <Audio uri={`${url.baseURL}${data.public_stem_audio}`} paused={paused} pausedEvent={this.audioPaused} ref={this.audio} />
                      : null
                  }
                  {exerciseInfo && exerciseInfo.public_stem_picture ? (
                    ...
                    <Image
                      ...
                      source={{ uri: url.baseURL + exerciseInfo.public_stem_picture }}
                    ></Image>
