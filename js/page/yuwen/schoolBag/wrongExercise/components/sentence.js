...
const Sentence = ({ typeIndex, navigation }) => {
  ...
  const getlist = async (pagenow) => {
    ...
    const res = await axios.get(
      `${api.getSentenceWrongKno}/${checkGrade}/${checkTeam}/${typeIndex}?page=${pagenow}`
    );
    // console.log("智能句", res.data);
    setloading(false);
          ...
          <View style={[styles.itemMainWrap]}>
            <View style={[styles.titleWrap]}>
              <Text style={[styles.titleTxt]}>
                {item.unit_name} {item.learning_name}
              </Text>
            </View>
            <Text style={[styles.itemTxt]}>{item.sentence}</Text>
          </View>
          <Image
