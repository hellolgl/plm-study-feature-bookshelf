...
class SpeSentenceExerciseTwo extends PureComponent {
  ...
  selectItem = (item, index) => {
    ...
    let indexnow = nowIndex + 1 === currentTopic.sentence_stem.length ? nowIndex : nowIndex + 1
    _currentTopic.sentence_stem[indexnow].isActive = true
    this.setState({
      currentTopic: _currentTopic,
      nowIndex: indexnow
  ...
  renderStem = () => {
    ...
    if (currentTopic.common_stem.indexOf('非顺序造句') === -1) {
      return <>
        <Text style={[{ fontSize: pxToDp(36), color: "#000" }, appFont.fontFamily_syst, fontFamilyRestoreMargin()]}>{currentTopic.common_stem}</Text>
        {currentTopic.stem ? (
    ...
    return (
      <View style={[styles.container]}>
              ...
              {currentTopic._change_word ? currentTopic._change_word.map((item, index) => {
                return <TouchableOpacity key={index} onPress={() => { this.selectItem(item, index) }} style={{ marginRight: pxToDp(20) }}>
                  <ImageBackground style={[styles.selectItem]} source={require("../../../../../images/select_bg.png")} resizeMode="stretch">
                    <Text style={[{ fontSize: pxToDp(36), color: '#fff', lineHeight: pxToDp(80) }, appFont.fontFamily_syst, fontFamilyRestoreMargin('', '', Platform.OS === 'ios' ? null : { marginTop: pxToDp(12) })]}>{item.content}</Text>
                  </ImageBackground>
                </TouchableOpacity>
              }) : null}
            </View>
                      ...
                      >
                        {currentTopic.sentence_stem
                          ? currentTopic.sentence_stem.map((item, index) => {
                            return <TouchableOpacity style={[styles.selectWrap, item.isActive ? { borderBottomColor: '#FC6161' } : null]} onPress={() => { this.clickSelect(item, index) }}>
                              <Text style={[{ fontSize: pxToDp(36) }, item.slectLabel ? { color: '#8E2904' } : { color: 'rgba(245, 245, 245, .5)' }, appFont.fontFamily_syst, fontFamilyRestoreMargin()]}>{item.slectLabel ? item.slectLabel : '请点击'}</Text>
                            </TouchableOpacity>
                          })
                          : null}
                      </View>
        ...
        <SentenceHelpModal
          ...
          doNumber={currentTopic.is_push === 1 || ranking === "0" ? 3 : 1}
