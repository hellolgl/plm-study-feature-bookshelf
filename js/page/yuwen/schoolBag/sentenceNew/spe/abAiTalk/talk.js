    ...
    useEffect(() => {
        ...
        const fetchData = async () => {
            if (!messages.length) {
                ...
                const data = { inspect: inspect, grade_code: grade_code, term_code: term_code, tags: tags };
                if (chatLoading === false) {
                    const history_res = await axios.post(api.getAbSenAIBotHistory, data);
                    ...
                    // });
                    try {
                        const res = await axios.post(api.getAbSentenceAiHelp, data);
