...
const Words = (props) => {
    ...
    const getList = async () => {
        const res = await axios.get(api.chinesDailyBagExercise, {
            params: {
        ...
        if (res.data.err_code === 0) {
            ...
            const { data, exercise_ids } = res.data.data;
            let statuslist = [];
            data.forEach(() => {
                statuslist.push("");
            });
            setstatusList(statuslist);
            setexercise_times_id(exercise_ids);
            setlist(data);
            settopaicNum(data.length);
        }
    };
    ...
    const saveExerciseDetail = async (exercise, answer) => {
        ...
        const data = {
            ...
            knowledge_point_code:
                fromServeCharacterList[listindex].knowledge_point_code,
            exercise_id: fromServeCharacterList[listindex].exercise_id,
            correction: yesOrNo, //批改对错，0 错误 1正确
            push_tag: fromServeCharacterList[listindex].push_tag, //是否需要推送题目
            knowledgepoint_type:
                fromServeCharacterList[listindex].knowledgepoint_type,
            exercise_level: fromServeCharacterList[listindex].exercise_level,
            exercise_element: fromServeCharacterList[listindex].exercise_element,
            push_num: fromServeCharacterList[listindex].push_num,
            exercise_ids: exercise_times_id,
            answer_origin: "3",
            exercise_set_id: fromServeCharacterList[listindex].exercise_set_id,
            exercise_result: answer,
            is_element_exercise: 1,
            // answer_start_time: this.state.answer_start_time,
            // answer_end_time: this.getTime(),
            is_finish: isFinish ? 0 : 1,
            unit_code: fromServeCharacterList[listindex].unit_code,
            is_seventh_push: fromServeCharacterList[listindex].is_seventh_push,
            textbook: "10",
            alias: "chinese_toDoExercise",
        };
        let correctnow = correct,
            wrongnow = wrong;
        const res = await axios.post(api.saveChineseBagExercise, data);
        if (res.data.err_code === 0) {
            // if (yesOrNo === 1) {
            ...
            if (res.data.data.exercise_id) {
                ...
                listnow.splice(listindex + 1, 0, insertObj);
                setlist(listnow);
                ...
                if (isFinish) {
                    ...
                    yesOrNo && dispatch(actionCreatorsUserInfo.getRewardCoin());
                    setlistindex(listindex + 1);
                }
                // isFinish ? getAnswerResult() : setlistindex(listindex + 1);
            }
            // }
        }
    };
