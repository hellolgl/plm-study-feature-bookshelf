...
class FlowWrongExerciseList extends PureComponent {
    ...
    toDoHomework = (exercise) => {
        ...
        if (data.name === '智能造句' && data.pName === '文化积累' || data.pName === '句型训练') {
            NavigationUtil.toDoWrongExerciseSmart(
                { ...this.props, data: { ...exercise, ...data, name: data.pName, inspect_name: data.name, type: 'Ai' } },
            );
        } else {
            NavigationUtil.toDoWrongExerciseSmart(
                { ...this.props, data: { ...exercise, ...data, name: data.pName, inspect_name: data.name } },
            );
        }
    ...
    getList = (page) => {
        ...
        let grade_term = this.info.checkGrade + this.info.checkTeam
        axios
            .get(`${api.getChineseSentenceAllExercise}`, {
                params: {
                    page,
                    iid: data.iid,
                    pName: data.pName,
                            ...
                            {this.state.fromServeCharacterList.length > 0 ? (
                                this.state.fromServeCharacterList.map((item, index) => {
                                    return (
                                        ...
                                        >
                                            <View style={{ flex: 1, flexDirection: 'row', alignItems: 'center', marginRight: pxToDp(40), }}>
                                                ...
                                                <View style={[{ flex: 1 }]}>
                                                    <Text style={[{ fontSize: pxToDp(32), marginBottom: pxToDp(20) }]}>{item.common_stem}</Text>
                                                    <Text style={{ fontSize: pxToDp(32), color: "#333330", width: '100%' }}>
