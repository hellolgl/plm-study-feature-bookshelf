...
TouchMatFractionIdx = ([row_idx, col_idx, int_idx, numerator_idx, denominator_idx]) => {
  ...
  render() {
    ...
    const {yindaoViewHeight,fitHeightNum,tip} = this.props
    // console.log('this.props.checkBtnArr',this.props.checkBtnArr)
    let style_height = {
       minHeight:fitHeight(0.391,0.51),
        maxHeight:fitHeight(0.391,0.51),
    }
    if(fitHeightNum) {
        style_height = {
          minHeight:fitHeight(fitHeightNum[0],fitHeightNum[1]),
          maxHeight:fitHeight(fitHeightNum[0],fitHeightNum[1]),
        }
    }
    return (
      <View style={[styles.container]}>
        <ScrollView  contentContainerStyle={{paddingBottom:pxToDp(40)}} style={[styles.content1,style_height,yindaoViewHeight?{minHeight:style_height.maxHeight-yindaoViewHeight,maxHeight:style_height.maxHeight-yindaoViewHeight}:null]}  ref={(view) => {if (view) this.myScrollView = view;}}>
