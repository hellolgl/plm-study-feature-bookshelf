...
// import { poly } from 'react-native/Libraries/Animated/Easing';
...
export default class DataGraphicalDisplay extends PureComponent {
    ...
    PolygonSvgCombine = (center_mat, radius, polygon_num, layer = 5, values_mat, namelist_mat) => {
        // 组装雷达多边形
        // 绘制基础框
        let polyon_svg = []
        for (let ii = 0; ii < layer; ii++) {
            let regular_polygon_mat = this.RegularPolygonMat(polygon_num, center_mat, [center_mat[0], center_mat[1] - radius / layer * (ii + 1)])
        ...
        values_mat.forEach((part_value, part_idx) => {
            let effect_radius = 0
            if (values_mat[part_idx].length < 1 || values_mat[part_idx] == 'null' || values_mat[part_idx] == null) {
                effect_radius = 0
            }
            else {
            ...
            effect_polygon_mat = this.RegularPolygonMat(polygon_num, center_mat, [center_mat[0], center_mat[1] - effect_radius])
        ...
        let effective_size = pxToDp(36)     // 有效率size
        let name_size = pxToDp(26)// 名称size
        ...
        let effect_polygon_loc_mat = this.RegularPolygonMat(polygon_num, center_mat, [center_mat[0], center_mat[1] - radius - base_radius])
        // 放置文本虚拟线
        ...
        effect_polygon_loc_mat.forEach((part_value, part_idx) => {
            // 修正区间排布：分区8份或者12份
            let [effect_start, name_start, new_frame_mat] = this.RadarSubZoneRemend(center_mat, part_value, namelist_mat[part_idx], values_mat[part_idx],
                name_size, [min_x, max_x, min_y, max_y])
            if (values_mat[part_idx].length < 1 || values_mat[part_idx] == 'null' || values_mat[part_idx] == null) {
                polyon_svg.push(<Text
    ...
    RadarSubZoneRemend = (center_mat, max_loc_mat, name_value, effect_value, font_size, frame_mat) => {
        ...
        else if (center_mat[0] == max_loc_mat[0] && center_mat[1] < max_loc_mat[1]) {
            radar_angle = 270
        }
        else {
            let radar_tan = (center_mat[1] - max_loc_mat[1]) / (max_loc_mat[0] - center_mat[0])
        ...
        // 返回两组值坐标：正确率和名称
        if (radar_angle < 105 && radar_angle >= 75) {
            console.log('上')
            ...
            // 求解区域
            let [min_x, max_x, min_y, max_y] = this.GetGraphSize(max_loc_mat, frame_mat, name_value, font_size, effect_x, effect_y, name_x, name_y)
            console.log('求解区域', min_x, max_x, min_y, max_y)
            return [[effect_x, effect_y], [name_x, name_y], [min_x, max_x, min_y, max_y]]
        }
        else if (radar_angle < 75 && radar_angle >= 45) {
            let name_x = 0.2 * font_size
            let name_y = -font_size * 0.1
            let effect_x = name_x + name_value.length / 2 * font_size - (effect_value.length + 1) / 2 * font_size * 6 / 11
        ...
        else if (radar_angle < 45 && radar_angle >= 15) {
        ...
        else if (radar_angle < 15 || radar_angle >= 345) {
            console.log('右')
            let name_x = 0.4 * font_size
        ...
        else if (radar_angle < 345 && radar_angle >= 315) {
            let name_x = 0.2 * font_size
        ...
        else if (radar_angle < 315 && radar_angle >= 285) {
            let name_x = -0.4 * font_size
            let name_y = font_size * 2
            let effect_x = name_x + name_value.length / 2 * font_size - (effect_value.length + 1) / 2 * font_size * 6 / 11
            let effect_y = name_y - font_size
            // 求解区域
            let [min_x, max_x, min_y, max_y] = this.GetGraphSize(max_loc_mat, frame_mat, name_value, font_size, effect_x, effect_y, name_x, name_y)
        ...
        else if (radar_angle < 285 && radar_angle >= 255) {
            console.log('下')
            let name_x = -name_value.length / 2 * font_size
        ...
        else if (radar_angle < 255 && radar_angle >= 225) {
        ...
        else if (radar_angle < 225 && radar_angle >= 195) {
            let name_x = -name_value.length * font_size + font_size * 0.5
            let name_y = font_size * 1.5
            let effect_x = name_x + name_value.length / 2 * font_size - (effect_value.length + 1) / 2 * font_size * 6 / 11
            let effect_y = name_y - font_size
            // 求解区域
        ...
        else if (radar_angle < 195 && radar_angle >= 165) {
            console.log('左')
            let name_x = -name_value.length * font_size - font_size * 0.3
            let name_y = font_size * 1
            let effect_x = name_x + name_value.length / 2 * font_size - (effect_value.length + 1) / 2 * font_size * 6 / 11
            let effect_y = name_y - font_size
            // 求解区域
        ...
        else if (radar_angle < 165 && radar_angle >= 135) {
        ...
        else if (radar_angle < 135 && radar_angle >= 105) {
    ...
    GetGraphSize = (max_loc_mat, frame_mat, name_value, font_size, effect_x, effect_y, name_x, name_y) => {
        // 最小最大x值
        ...
        if ((max_loc_mat[0] + name_x + name_value.length * font_size) < frame_mat[1]) {
            max_x = frame_mat[1]
        }
        else {
    ...
    EffectiveAreaSvg = (center_mat, radius, polygon_num, values_mat) => {
        ...
        values_mat.forEach((part_value, part_idx) => {
            ...
            regular_polygon_mat = this.RegularPolygonMat(polygon_num, center_mat, [center_mat[0], center_mat[1] - effect_radius])
        ...
        polyon_svg.push(<Polygon
            ...
            strokeWidth="2"   //外边框宽度
        />)
        return (<Svg height="900" width="900" backgroundColor='red'>
            {polyon_svg}
        </Svg>
        )
    }

    PolygonSvgCombine2 = (center_mat, radius, polygon_num, layer = 5, values_mat, namelist_mat, average_mat) => {
        // 组装雷达多边形
        // 绘制基础框
        let polyon_svg = []
        for (let ii = 0; ii < layer; ii++) {
        ...
        average_mat.forEach((part_value, part_idx) => {
            let effect_radius = 0
            if (average_mat[part_idx].length < 1 || average_mat[part_idx] == 'null' || average_mat[part_idx] == null) {
                effect_radius = 0
            }
            else {
                effect_radius = radius * eval(part_value) / 100
            ...
            // console.log('eval(part_value)', eval(part_value), effect_radius)
            average_polygon_mat = this.RegularPolygonMat(polygon_num, center_mat, [center_mat[0], center_mat[1] - effect_radius])
            // console.log('effect_polygon_mat[part_idx]', effect_polygon_mat[part_idx])
    ...
    DottedLineMat = (loc_mat_1, loc_mat_2) => {
        ...
        let all_length = Math.sqrt((loc_mat_1[0] - loc_mat_2[0]) * (loc_mat_1[0] - loc_mat_2[0]) +
            (loc_mat_1[1] - loc_mat_2[1]) * (loc_mat_1[1] - loc_mat_2[1]))
        let dotted_length = all_length / dotted_num        // 单组虚线长度
        let [loc_x_mat, loc_y_mat, loc_all_mat] = this.LineDiscreteData(loc_mat_1, loc_mat_2)
        ...
        for (let idx = 0; idx < dotted_num; idx++) {
            if (idx == 0) {
                ...
                dotted_mat.push([loc_x_mat[loc_x_mat.length - 1], loc_y_mat[loc_y_mat.length - 1]])
            }
            else {
    ...
    RadarMap = (values_mat, namelist_mat, radius) => {
        ...
        let layer = 4
        // 绘制基础框
        let [[min_x, max_x, min_y, max_y], polyon_svg_mat] = this.PolygonSvgCombine([center_x, center_y], radius, polygon_num, layer, values_mat, namelist_mat)
        // // 绘制有效区域
        // let effect_svg_mat = this.EffectiveAreaSvg([center_x, center_y], radius, polygon_num, values_mat)
        return (<View height={max_y - min_y + 40} width={max_x - min_x + 20} style={styles.radar_view}>
            {polyon_svg_mat}
        </View>
        )
    }

    RadarMap2 = (values_mat, namelist_mat, radius, [radar_width, radar_height]) => {
        // 雷达图展示
        let [center_x, center_y] = [300, 300]
        // let radius = 120
        let polygon_num = values_mat.length
        ...
        if (init_height + 25 <= radar_height && init_width + 5 <= radar_width) {
            return (<View height={max_y - min_y + 25} width={max_x - min_x + 5} style={styles.radar_view}>
                {polyon_svg_mat}
            </View>
            )
        }
        // 计算多边形的最大有效区域
        let regular_polygon_mat = this.RegularPolygonMat(polygon_num, [center_x, center_y], [center_x, center_x - radius])
        console.log('regular_polygon_mat', JSON.stringify(regular_polygon_mat))
        let [init_min_x, init_max_x, init_min_y, init_max_y] = [10000, -10000, 10000, -10000]
        ...
        // 固定大小修正
        let [[min_x2, max_x2, min_y2, max_y2], polyon_svg_mat2] = this.PolygonSvgCombine([center_x, center_y], new_radius, polygon_num, layer, values_mat, namelist_mat)
        // let effect_svg_mat = this.EffectiveAreaSvg([center_x, center_y], radius, polygon_num, values_mat)
        return (<View height={max_y2 - min_y2 + 25} width={max_x2 - min_x2 + 5} style={styles.radar_view}>
            {polyon_svg_mat2}
        </View>
        )
    }
    ...
    RadarMap3 = (values_mat, namelist_mat, radius, average_mat) => {
        ...
        let layer = 5
        // 绘制基础框
        let [[min_x, max_x, min_y, max_y], polyon_svg_mat] = this.PolygonSvgCombine2([center_x, center_y], radius, polygon_num, layer, values_mat, namelist_mat, average_mat)
        // // 绘制有效区域
        // let effect_svg_mat = this.EffectiveAreaSvg([center_x, center_y], radius, polygon_num, values_mat)
        return (<View height={max_y - min_y + 40} width={max_x - min_x + 20} style={styles.radar_view}>
            {polyon_svg_mat}
    ...
    render() {
        ...
        if (this.props.math_frame_svg[0] == 'radar_mode1') {
            ...
            let namelist_mat = this.props.math_frame_svg[2]
            let radius = this.props.math_frame_svg[3]
            let [radar_width, radar_height] = [this.props.math_frame_svg[4][0], this.props.math_frame_svg[4][1]]
            // console.log('values_mat', values_mat)
            // console.log('namelist_mat', namelist_mat)
            return this.RadarMap2(values_mat, namelist_mat, radius, [radar_width, radar_height])
        }
