...
export default class MathFrameSynthesisView extends PureComponent {
    constructor(props) {
        super(props);
        this.state = {
            //用于更新界面
    ...
    getgroupbinaryloc = (parent_mat, children_width_mat, children_height_mat, symbol_width_mat, symbol_height_mat, gap_mat, global_mat, group_color)=>{
        ...
        unit_binary_mat.parentTochildren.parent_loc = []    // 上父下子结构数据:父组件坐标 unit_binary_mat.group_height
        unit_binary_mat.parentTochildren.parent_loc.box_center_loc = [unit_binary_mat.group_width/2,unit_binary_mat.group_height-unit_binary_mat.parent.height/2+1]   // center_x, center_y
        unit_binary_mat.parentTochildren.parent_loc.up_center_loc = [unit_binary_mat.group_width/2, unit_binary_mat.group_height-unit_binary_mat.parent.height]   // up_center_x, up_center_y  // 修改父组坐标
        unit_binary_mat.parentTochildren.parent_loc.down_center_loc = [unit_binary_mat.group_width/2, unit_binary_mat.group_height-1]   // down_center_x, down_center_y//框图法修改父组坐标
        unit_binary_mat.parentTochildren.parent_loc.box_start_loc = [unit_binary_mat.group_width/2-unit_binary_mat.parent.width/2+1, unit_binary_mat.group_height-unit_binary_mat.parent.height]   // 起始坐标
        // 依次循环计算子框坐标
        ...
        for (let ii=0;ii<unit_binary_mat.children.width.length;ii++){
          ...
          if(ii==0){
            ...
            part_box_center_loc_y = max_children_height/2                  // 全局高度-最大子组件高度/2
            ...
            part_box_center_loc_y = max_children_height/2
            ...
            symbol_part_box_center_loc_y = max_children_height/2
    ...
    getgroupbinarySVG = (unit_binary_mat, fill_color)=>{
        ...
        for(let ii=0;ii<unit_binary_mat.parentTochildren.children_loc.box_start_loc.length;ii++){
          ...
          let line_binary = <Line x1={unit_binary_mat.parentTochildren.parent_loc.up_center_loc[0]+unit_binary_mat.global_x} 
                                  y1={unit_binary_mat.parentTochildren.parent_loc.up_center_loc[1]+unit_binary_mat.global_y} 
                                  x2={unit_binary_mat.parentTochildren.children_loc.down_center_loc[ii][0]+unit_binary_mat.global_x} 
                                  y2={unit_binary_mat.parentTochildren.children_loc.down_center_loc[ii][1]+unit_binary_mat.global_y} 
                                  stroke={unit_binary_mat.color} 
                                  strokeWidth='2'
                                  ></Line>
    ...
    render() {
        ...
        let all_global_x = 830;
        let all_global_y = 820;
        ...
        // console.log('组装情况', algebra_text_mat,used_algebra, using_algebra, to_use_algebra)
        ...
        while (svg_flag>=0  && svg_count_num<100 ){
            ...
            for (let ii=0; ii<algebra_process.level_mat.length-1;ii++){
                if(ii==0){  // 顶层
                    ...
                    // console.log('全局赋值---顶层', algebra_process)
                    // svg_flag = -1
                    // break
                    svg_count_num += 1
                }
