...
class ChineseMindMapping extends Component {
  ...
  componentWillMount() {
    ...
    for (let part_key in this.props.mindmapping) {
      if (this.props.mindmapping[part_key].parent.length < 1) {
        // 根节点
        view_color_mat[part_key] = show_color;
        view_show_mat[part_key] = true;
        show[part_key] = false;
      } else {
        // 子节点
        view_color_mat[part_key] = hidden_color;
        view_show_mat[part_key] = false;
        show[part_key] = false;
      }
    }
    ...
    this.setState({
      view_show_mat: view_show_mat,
      view_color_mat: view_color_mat,
      showView: show,
    ...
    if (JSON.stringify(state.view_show_mat) === "{}") {
      for (let part_key in props.mindmapping) {
        if (props.mindmapping[part_key].parent.length < 1) {
          // 根节点
          tempState.view_color_mat[part_key] = show_color;
          tempState.view_show_mat[part_key] = true;
          tempState.showView[part_key] = false;
        } else {
          // 子节点
          tempState.view_color_mat[part_key] = hidden_color;
          tempState.view_show_mat[part_key] = false;
          tempState.showView[part_key] = false;
        }
      }
    } else {
  ...
  ChineseMindView = (do_exercises_dict) => {
    ...
    for (let part_key in all_global_loc) {
      ...
      // 最大的底层
      let show = this.state.showView[part_key];
      part_svg_mat.push(
        this.state.view_show_mat[part_key] ? (
          <TouchableOpacity
            style={{
              ...
              backgroundColor: show ? "#F07C39" : "#323F56",
              borderRadius: 20,
              // borderColor: '#fff',
              // borderWidth: 4,
              position: "absolute",
          ...
          >
            <View
              style={{
                flex: 1,
                backgroundColor: show ? "#FF964A" : "#4D5B75",
                borderRadius: 20,
  ...
  TextMat = (part_key0) => {
    ...
    let view_color_mat = this.state.view_color_mat;
    let view_show_mat = this.state.view_show_mat;

    // 修正所有图像的颜色
    view_color_mat[part_key0] = show_color;
    let choice_level = this.level_mat[part_key0]; // 选择层级
    // 当前选择等级下的全部为隐藏色
    for (let part_key in this.level_mat) {
      if (this.level_mat[part_key] > choice_level) {
        view_color_mat[part_key] = hidden_color;
        view_show_mat[part_key] = false; // 隐藏子组件
      }
    }
    ...
    let show = this.state.showView;
    if (children_mat.length > 0) {
      for (let i in show) {
        show[i] = false;
      }
      show[part_key0] = children_mat.length > 0;
    }

    // console.log("children_mat", children_mat);
    for (let part_idx in children_mat) {
      // console.log("part_str", children_mat[part_idx]);
      view_color_mat[children_mat[part_idx]] = show_color;
      view_show_mat[children_mat[part_idx]] = true; // 显示
    }
    // console.log("this.view_show_mat", view_show_mat);
    this.setState({
      view_color_mat: view_color_mat,
      view_show_mat: view_show_mat,
      showView: show,
